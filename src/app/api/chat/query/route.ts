...
function analyzeIntent(message: string, _keywords: string[]): string {
...
async function generateResponse(message: string, context: QueryContext, history: Message[]): Promise<{
  ...
  const { matrices, relevant_cells } = context;
...
async function generateLLMResponse(message: string, context: QueryContext, _history: Message[]): Promise<{
  content: string;
  confidence: number;
}> {
...
function _generateInformationResponse(cells: any[], matrices: any[]): string {
  if (cells.length === 0) {
...
function _generateExplanationResponse(cells: any[], _matrices: any[], _message: string): string {
  if (cells.length === 0) {
...
function _generateComparisonResponse(cells: any[], _matrices: any[]): string {
  if (cells.length < 2) {
...
function _generateMatrixLookupResponse(cells: any[], matrices: any[], message: string): string {
...
function _generateGeneralResponse(cells: any[], matrices: any[], message: string): string {
...
function _generateFallbackResponse(matrices: any[]): string {
